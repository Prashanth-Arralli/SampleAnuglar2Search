...
export class SearchService {
  ...
  getSearchList(key: string, startIndex: number, maxLimit: number, years: string[], doctypes: string[]): Observable<ResultType> {
    let filterParam = `${years.length? `&year=${years}`: ``}${doctypes.length? `&doctype=${doctypes}`: ``}`;
  ...
  findByDocId(id: string): Observable<Search[]> {
    return this.http.get<Search[]>(`/api/search/${id}`, {
    headers: {
        "Authorization": `Token token=${this.hashCode()}`
    }
    });
